...
// recompute a motor's live fields when frequency changes (for "apply suggestion")
function applyFreq(motor, freq) {
  const op = window.operatingPoint(motor.pump, freq);
  const running = freq > 0;
  return {
    ...motor, freq, recFreq: freq, save: 0,
    power_kw: running ? op.P : 0,
    flow_m3h: running ? op.Q : 0,
    ...
    sec_kwh_m3: running && op.Q ? +(op.P / op.Q).toFixed(3) : 0,
  };
}
  ...
  return (
    <div style={{
      ...
      <div style={{ padding: '13px 15px' }}>
        ...
        {!applied ? (
          <>
            <div style={{ fontSize: 13.5, fontWeight: 700, color: BP.label, marginBottom: 4 }}>{g.title}</div>
            <div style={{ fontSize: 12, color: BP.text, lineHeight: 1.5 }}>{g.detail}</div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 9, padding: '8px 11px', borderRadius: 8, background: `${lv.c}15`, border: `1px solid ${lv.c}44` }}>
              <span style={{ fontFamily: BP.mono, fontSize: 13, fontWeight: 700, color: BP.label }}>{g.action}</span>
              {g.save > 0 && <span style={{ marginLeft: 'auto', fontFamily: BP.mono, fontSize: 13, fontWeight: 700, color: '#22C55E' }}>省 {g.save}%</span>}
            </div>
            <div style={{ display: 'flex', gap: 8, marginTop: 11 }}>
              <button onClick={() => onApply && onApply(g)} style={btn(BP, true)}>套用建議</button>
              <button onClick={() => onGoto && onGoto('alerts')} style={btn(BP, false)}>查看詳情</button>
...
function AIAssistant({ summary }) {
  ...
  const [msgs, setMsgs] = React.useState([{ role: 'ai', text: '您好，我是配水加壓站的節能分析助理。我可以查詢即時機組狀態、能耗、異常原因，或給降頻節能建議。請問需要什麼協助？', src: [] }]);
      ...
      <window.BPCard title="智慧問答助理" en="AI Assistant · RAG" glow
        ...
        <div style={{ display: 'flex', gap: 9, padding: '11px 14px', borderTop: `1px solid ${BP.borderDim}` }}>
          <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') send(); }}
            placeholder="以中文提問，例如：P-303 為什麼能耗偏高？" style={{
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12, minHeight: 0, overflow: 'auto' }}>
        <window.BPCard title="本日 AI 重點" en="Daily Brief" glow>
          <div style={{ padding: 13, fontSize: 12.5, color: BP.label, lineHeight: 1.6 }}>
            全站節能率 <b style={{ color: '#22C55E', fontFamily: BP.mono }}>{summary.saving_pct}%</b>；P-303 能耗偏高待處理，建議降頻 45Hz（省 10.6%）。本月已節省 <b style={{ color: '#7cd4ff', fontFamily: BP.mono }}>{(summary.month_saved_kwh / 1000).toFixed(1)} MWh</b>。
          </div>
        </window.BPCard>
        <window.BPCard title="可查詢的資料來源" en="Knowledge Base">
          <div style={{ padding: '6px 13px 12px' }}>
            {[['即時製程資料', 'OPC UA / MQTT Tag'], ['能效模型', 'M-015 黃金曲線'], ['泵浦效率曲線', 'P-301~304'], ['維護 SOP', '軸承 / 振動 / 巡檢'], ['操作手冊', '變頻 / 故障排除'], ['能源日報', '本月彙總']].map((r, i) => (
        ...
        <window.BPCard title="安全邊界" en="Guardrail">
          <div style={{ padding: 13, fontSize: 11.5, color: BP.text, lineHeight: 1.6 }}>
            AI 僅<b style={{ color: BP.label }}>提供建議與分析</b>，不會自動變更 PLC 或設備參數。實際降頻需由具權限人員於「操作建議」確認執行。
          </div>
        </window.BPCard>
      </div>
    </div>
...
// ============================================================================
function OperatorGuidance({ onApply }) {
  const BP = window.BP;
  const vp = window.useVP ? window.useVP() : { isMobile: false };
              ...
              <div style={{ padding: '12px 14px' }}>
                ...
                {g.action !== '—' && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 6 }}>
                    <span style={{ fontFamily: BP.mono, fontSize: 13, fontWeight: 700, color: BP.label }}>{g.action}</span>
                    {g.save > 0 && <span style={{ fontFamily: BP.mono, fontSize: 12.5, fontWeight: 700, color: '#22C55E' }}>省 {g.save}% ≈ {g.savekw} kW</span>}
                    {!isDone && g.status !== '已解除' && g.status !== '已確認' && (
                      <div style={{ display: 'flex', gap: 8, marginLeft: 'auto' }}>
                        {g.save > 0 && <button onClick={() => { onApply && onApply(g); setDone(d => ({ ...d, [g.id]: true })); }} style={btn(BP, true)}>套用建議</button>}
                        <button onClick={() => setDone(d => ({ ...d, [g.id]: true }))} style={{ ...btn(BP, false), flex: '0 0 auto', padding: '8px 14px' }}>確認</button>
                      </div>
                    )}
                    {isDone && <span style={{ marginLeft: 'auto', fontFamily: BP.mono, fontSize: 12, color: '#22C55E' }}>✓ 已處理</span>}
                  </div>
                )}
              </div>
            </window.BPCard>
