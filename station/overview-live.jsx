...
const LIVE_METRICS = {
  power: { key: 'power_kw', unit: 'kW', band: 0.05, noise: 0.012, fmt: v => Math.round(v).toLocaleString(), label: '功率', baseline: true },
  flow: { key: 'flow_m3h', unit: 'm³/h', band: 0.045, noise: 0.011, fmt: v => Math.round(v).toLocaleString(), label: '流量', baseline: false },
  eff: { key: 'eff_pct', unit: '%', band: 0.03, noise: 0.006, fmt: v => v.toFixed(1), label: '效率', baseline: false },
};
...
function CombinedLive({ motors, cfg, metric }) {
  ...
  const targetTotal = running.reduce((s, m) => s + (m[cfg.key] || 0), 0) || 0.001;
  const baseTotal = cfg.baseline ? running.reduce((s, m) => s + (m.baseline_kw || 0), 0) : null;
  ...
  return (
    <div style={{ background: 'rgba(8,21,44,.5)', border: `1px solid ${BP.borderDim}`, borderRadius: 9, padding: '10px 12px 7px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
        ...
        <span style={{ fontFamily: BP.mono, fontSize: 12.5, fontWeight: 700, color: BP.label }}>全機組合計 · {cfg.label}</span>
        ...
        {savePct != null && <span style={{ fontFamily: BP.mono, fontSize: 11.5, fontWeight: 700, color: '#22C55E', background: 'rgba(34,197,94,.14)', padding: '2px 8px', borderRadius: 4 }}>↓ 較基準省 {savePct.toFixed(1)}%</span>}
      </div>
      <div ref={ref} style={{ marginTop: 4 }}>
        <svg viewBox={`0 0 ${w} ${H}`} width="100%" height={H} style={{ display: 'block', fontFamily: BP.mono }}>
          ...
          {baseLine && (<>
            ...
            <text x={X(N_LIVE - 1)} y={Y(baseLine[baseLine.length - 1]) - 5} textAnchor="end" fontSize="9" fill="#F59E0B">基準線 {cfg.fmt(baseTotal)}</text>
          </>)}
  ...
  return (
    <window.BPCard title="即時趨勢監控 · 全機組" en="Live Trend · All Running Units" glow
      ...
      <div style={{ padding: '0 12px 11px', display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap', fontSize: 10.5, color: BP.textDim, fontFamily: BP.mono }}>
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5 }}><span style={{ width: 14, height: 0, borderTop: '2px dashed #F59E0B' }} />基準線（未優化）</span>
        ...
        <span style={{ marginLeft: 'auto' }}>每 1.5 秒更新 · 色帶＝合理運轉通道（±{(cfg.band * 100).toFixed(0)}%）</span>
      </div>
    </window.BPCard>
  );
}
